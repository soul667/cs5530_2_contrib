...
class PennSearch : public PennApplication
{
  ...
  private:
    ...
    std::map<std::string,std::vector<std::string>> m_invertedLists;  
    std::vector <std::string> m_termList;
    
    // 处理搜索请求
    void HandleSearchRequest(uint32_t targetNode, std::vector<std::string>& searchTerms);
    
    // 处理搜索响应
    void ProcessSearchResponse(PennSearchMessage message, Ipv4Address sourceAddress, uint16_t sourcePort);
    
    // 存储搜索结果的成员变量
    std::map<uint32_t, std::vector<std::string>> m_searchResults;
};
