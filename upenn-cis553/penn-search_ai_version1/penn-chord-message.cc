...
PennChordMessage::Deserialize(Buffer::Iterator start)
{
  ...
  case PING_RSP:
    ...
    break;
  case LOOKUP_MSG:
    size += m_message.lookupMsg.Deserialize(i);
    break;
  default:
    NS_ASSERT(false);
  }
...
{
  uint32_t size = sizeof(uint16_t) + key.length() + IPV4_ADDRESS_SIZE + sizeof(uint16_t) + msgType.length();
  return size;
}

void PennChordMessage::LookupMsg::Print(std::ostream &os) const
{
  os << "LookupMsg:: Key: " << key << ", OriginNode: " << originNode << ", MsgType: " << msgType << "\n";
}

void PennChordMessage::LookupMsg::Serialize(Buffer::Iterator &start) const
{
  start.WriteU16(key.length());
  start.Write((uint8_t *)(const char *)(key.c_str()), key.length());

  uint8_t buf[4];
  originNode.Serialize(buf);
  start.Write(buf, 4);

  start.WriteU16(msgType.length());
  start.Write((uint8_t *)(const char *)(msgType.c_str()), msgType.length());
}
...
PennChordMessage::LookupMsg::Deserialize(Buffer::Iterator &start)
{
  ...
  char *keyStr = (char *)malloc(keyLength);
  start.Read((uint8_t *)keyStr, keyLength);
  key = std::string(keyStr, keyLength);
  free(keyStr);

  uint8_t buf[4];
  start.Read(buf, 4);
  originNode.Deserialize(buf);

  uint16_t msgTypeLength = start.ReadU16();
  char *msgTypeStr = (char *)malloc(msgTypeLength);
  start.Read((uint8_t *)msgTypeStr, msgTypeLength);
  msgType = std::string(msgTypeStr, msgTypeLength);
  free(msgTypeStr);

  return GetSerializedSize();
}

void PennChordMessage::SetLookupMsg(std::string key, Ipv4Address originNode, std::string msgType)
{
  if (m_messageType == 0)
  {
  ...
  m_message.lookupMsg.originNode = originNode;
  m_message.lookupMsg.msgType = msgType;
}
